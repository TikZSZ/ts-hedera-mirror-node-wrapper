...
import { accounts, networkSupply, optionalFilters, topicMessages, 
  TransactionType, transactions, tokenUtils, smartContract, 
  TokenTypeFilter, nftUtils, NFTUtils, TokenUtils, Client,
} from "../src";
...
it("should get accounts", async () => {
  const accountCursor = accounts(client).setLimit(2).order("desc");
...
describe("transactions", async () => {
  it("should get transfer txns", async () => {
    ...
    it("should get credit txns", async () => {
      ...
      expect(
        txns.transactions[0].transfers.find((txn) => {
    ...
    it("should get debit txns", async () => {
      ...
      expect(
        txns2.transactions[0].transfers.find((txn) => {
          if (txn.account === accountId) {
            return txn.amount;
          }
        })?.amount
  ...
  it("should get consensus msg txns", async () => {
    ...
    const txns = await transactionCursor
      ...
      .setLimit(1)
      .get();
    expect(txns.transactions[0].name).toEqual(
